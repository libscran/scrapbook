...
This allows us to summarize our complex scRNA-seq data into discrete categories for easier human interpretation.
Specifically, a biological meaning is assigned to each cluster, typically based on its upregulated marker genes (Chapter \@ref(marker-detection)).
...
We build a "shared nearest neighbor" (SNN) graph from the PCs of our dataset.
Two cells are connected by an edge if they share any of their nearest neighbors,
where the weight of the edge is defined from the number or rank of the shared neighbors [@xu2015identification].
We then apply a community detection algorithm on the graph - in this case the multi-level algorithm, also known as Louvain.
...
Again, if we're not satisfied with the results, we can just tinker with the parameters.
Obviously, we could just increase $k$ to obtain a greater numbr of smaller clusters. 
...
# Using centroids for something expensive, e.g., hierarchical clustering.
...
cutree.vq.10x <- cutreeDynamic(hclust.vq.10x, distM=as.matrix(dist.vq.10x), minClusterSize=1)
table(cutree.vq.10x)
...
These thoughts have plagued us ever since we did our first scRNA-seq analysis^[
...
Nonetheless, if we really need some kind of "objective" metric of cluster quality^[
...
bootstrap.10x <- bootstrapStability(
    pcs.10x$components,
    FUN=function(x) {
        # Replace with your clustering procedure 
        g <- buildSnnGraph(x)
...
we can choose from a variety of measures of cluster "quality":

- The silhouette width, as implemented in the `approxSilhouette()` function from `r Biocpkg("bluster")`.
...
- The clustering purity, as implemented in the `clusterPurity()` function from `r Biocpkg("bluster")`.
...
- The root mean-squared deviation (RMSD), as implemented in the `clusterRSMD()` function from `r Biocpkg("bluster")`.
...
One might be tempted to objectively define a "best" clustering by fiddling with the clustering parameters to optimize one of these metrics, e.g., maximum silhouette width. 
