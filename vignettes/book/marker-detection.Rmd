...
Uninteresting factors of variation in our dataset can interfere with marker gene detection.
They can inflate the variance within each cluster but also distort the log-fold changes if the cluster composition varies across levels of the factor(s).
To avoid these issues, we block on any uninteresting factors when computing the effect sizes.
Let's demonstrate on a mouse trophoblast dataset [@lun2017assessing] generated across two microwell plates:
...
We set `block=` to instruct `scoreMarkers()` to perform the pairwise comparisons separately in each plate.
By comparing within each batch, we cancel out any batch effects so that they are not conflated with biological differences between clusters.
This also involves inflation of variances within clusters by not combining cells from multiple batches. 
The effect sizes are then averaged across batches to obtain a single value per comparison,
using a weighted mean that accounts for the number of cells involved in the comparison in each batch.
A similar correction is applied to the mean log-expression and proportion of detected cells. 
