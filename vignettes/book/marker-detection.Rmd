...
ordered.markers.1.tropho <- markers.1.tropho[order(markers.1.tropho$cohens.d.mean, decreasing=TRUE),]
head(ordered.markers.1.tropho[,c("mean", "detected", "delta.mean.mean", "cohens.d.mean")], 10)
```
...
```{r markers-violin-batch, fig.wide=TRUE, fig.cap=paste0("Distribution of expression values for the top-ranked marker gene (", rownames(ordered.markers.1.tropho)[1], ") of cluster 1 in the trophoblast dataset. Distributions are shown for each cluster (x-axis) in each batch (panel).")}
plotExpression(sce.qc.tropho, features=rownames(ordered.markers.1.tropho)[1], x="clusters", other_fields="block") +
    facet_grid(~block)
```
...
library(scrapper)
enrich.chosen.10x <- testEnrichment(top.markers.chosen.10x, go.bp.sets, universe=rownames(ordered.chosen.10x))
