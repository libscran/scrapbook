...
For simplicity, we do not explicitly penalize genes that behave inconsistently across blocks.
...
We can check this by visualizing the expression profile of each gene of interest with respect to the plate (Figure \@ref(fig:markers-violin-block)).
Ideally, each gene would behave consistently across plates as this is the easiest to interpret -
namely, any conclusions about the biological identity of a cluster will be applicable to all plates.

```{r markers-violin-block, fig.wide=TRUE, fig.cap=paste0("Distribution of expression values for the top-ranked marker gene (", rownames(markers.tropho[["1"]])[1], ") of cluster 1 in the trophoblast dataset. Distributions are shown for each cluster (x-axis) in each plate (panel).")}
plotExpression(
    sce.nn.tropho,
