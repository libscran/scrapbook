...
We compute thresholds using the outlier-based strategy described in Section \@ref(quality-control-outlier) (Figure \@ref(fig:qc-dist-adt-pbmc)).
...
If we wanted to use custom thresholds, we could modify our thresholds in the same manner as described in Section \@ref(fixed-qc).
...
Once we have our (centered) size factors, we can compute log-normalized abundance values for ADTs as described in Section \@ref(normalization-transformation).
...
This summarizes the heterogeneity specific to the ADT modality (Figure \@ref(cite-tsne)).
...
hvg.rna.pbmc <- chooseHighlyVariableGenes(var.rna.pbmc$residuals, top=3000)
...
```{r}
scaled.combined.pbmc <- scaleByNeighbors(list(RNA=pcs.rna.pbmc$components, ADT=pcs.adt.pbmc))
...
plotReducedDim(sce.main, "TSNE.combined", colour_by="clusters.combined")
```
