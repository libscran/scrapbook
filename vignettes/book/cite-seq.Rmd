...
```{r qc-dist-adt-pbmc, fig.width=10, fig.height=5, fig.cap="Distribution of ADT-based QC metrics in the PBMC dataset. Each point represents a cell, while dashed lines represent thresholds for each metric."}
...
We'll start by considering the simplest possible method of combining information across modalities, i.e., literally combining their PC score matrices. 
(If we didn't perform PCA on the ADTs, the log-abundance matrix could be used instead.)
...
To balance the contributions of each modality to the combined matrix, we scale their respective matrices with the `scaleByNeighbors.se()` function.
