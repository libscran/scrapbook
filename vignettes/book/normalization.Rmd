...
Our assumption is that any cell-specific bias affects all genes equally by scaling their expected counts in that cell.
...
Strictly speaking, library size normalization is not correct due to composition bias in the presence of differential expression (DE). 
Imagine that gene $X$ is strongly upregulated in a particular cell, which increases its total RNA content and thus its library size.
Upon normalization, the increased library size deflates the apparent expression of all other genes in the same cell.
This results in spurious DE for those genes when compared to other cells that do not upregulate $X$.
...
- Composition biases are not introduced if the DE is "balanced",
  i.e., any upregulation for a subset of genes is cancelled out by the same magnitude of downregulation in a different subset of genes.
  Anecdotally, many perturbations seem to introduce balanced DE, presumably because there are only so many genes that a cell can express at any given time.
...
If we really want to remove composition biases, we'll need to use more sophisticated methods [@robinson2010scaling;@anders2010differential;@lun2016pooling].
But it's probably not worth it^[
As much as it pains me to admit it.
Accurate normalization goes into the bin of "things we thought were important at the time", next to all those zero-inflated models.].
...
The log-transformation is useful as the difference between log-values is an estimate of the log-fold change in normalized expression.
When comparing expression values, log-fold changes are usually preferable to absolute differences in expression.
For example, which one is more interesting - a gene that is expressed at an average count of 50 in cell type $A$ and 10 in cell type $B$,
...
Our interpretation of the difference in log-values is important in downstream procedures based on distances between cells, which includes many clustering and dimensionality reduction algorithms.
By operating on log-transformed data, we ensure that the distances reflect log-fold changes in expression,
e.g., the Euclidean distance between two cells is the root of the sum of squared log-fold changes across all genes.
...
For example, given a pseudo-count of 1, the estimated log-fold change between normalized expression values of 1 and 2 would be $\log_2((2 + 1)/(1 + 1)) \approx 0.58$,
while the log-fold change between normalized expression values of 10 and 20 would be $\log_2((20 + 1)/(10 + 1)) \approx 0.93$.
Shrinkage is desirable as it reduces the influence of log-differences derived from low counts, which are less reliable due to random sampling noise. 
By comparison, genes with larger counts are less affected by the addition of the pseudo-count and will contribute more to the distances between cells.
...
```{r zeisel-demo-snap25, fig.width=8, fig.height=5, fig.cap="Distribution of log-expression values for _Snap25_ in each cell type of the Zeisel brain dataset."}
library(scater)
...
To illustrate, let's look at two brain datasets generated with different scRNA-seq technologies [@zeisel2015brain;@tasic2016adult]
where the average library sizes differ by several orders of magnitude (Figure \@ref(fig:norm-batch)).
If we scaled up the Zeisel counts by ~100-fold, their normalized expression values would become much larger than the usual pseudo-count.
This would weaken the shrinkage effect (Section \@ref(norm-transformation)) and increase the impact of random noise on the various log-value calculations.
...
A more conservative approach is to scale down all batches to match the coverage of the lowest-coverage batch.
We specify the study of origin for each cell in `block=`, which adjusts the size factors to scale the Tasic cells down to the average coverage of the Zeisel cells (Figure \@ref(fig:norm-batch-sf)).
This avoids amplication of sampling noise at the cost of greater shrinkage of the Tasic counts to match that of the Zeisel data.
In effect, we're forgoing some of the improved accuracy offered by deeper coverage in the Tasic study to make it more comparable to the low-coverage Zeisel study.
Hopefully, there should still be plenty of biological signal in higher-abundance genes that are less affected by shrinkage.
...
We compute the total count across all spike-in transcripts in each cell and center them to a mean of 1 across all cells (see Section \@ref(norm-centering)) to obtain spike-in size factors.
We scale each cell's counts by its corresponding spike-in size factor to generate normalized expression values. 
For convenience, this entire process is handled by the `normalizeRnaCountsWithSpikeIns.se()` function.
...
# The spike-in counts are stored in an alternative experiment.
altExp(sce.qc.richard, "ERCC")

library(scrapper)
sce.spikenorm.richard <- normalizeRnaCountsWithSpikeIns.se(
    sce.qc.richard,
    spike.altexps="ERCC", # compute spike-in factors with the ERCC counts.
    use.spike.ins.for.endogenous=TRUE # use spike-in factors on the endogenous genes.
)

spike.factor.richard <- sce.spikenorm.richard$sizeFactor
summary(spike.factor.richard)
hist(log10(spike.factor.richard), xlab=expression('Log'[10]*'(Size factor)'), col='grey80')
```
...
The differences between these two normalization strategies are illustrated in Figure \@ref(fig:norm-spike-t).
The spike-in and library size factors are positively correlated within each treatment condition,
...
```{r norm-spike-t, fig.height=8, fig.width=10, fig.cap="Size factors from spike-in normalization, plotted against the library size factors for all cells in the T cell dataset. Each plot represents a different ligand treatment and each point is a cell coloured according by time from stimulation."}
# Performing library size normalization for comparison. 
sce.libnorm.richard <- normalizeRnaCounts.se(sce.qc.richard)
lib.factor.richard <- sce.libnorm.richard$sizeFactor
to.plot <- data.frame(
...
The differences between the factors have real consequences for downstream interpretation.
...
```{r norm-effect-malat, fig.height=8, fig.width=10, fig.cap="Distribution of log-normalized expression values for _Malat1_ after normalization with the deconvolution size factors (left) or spike-in size factors (right). Cells are stratified by the ligand affinity and colored by the time after stimulation."}
library(scater)
gridExtra::grid.arrange(
    plotExpression(sce.libnorm.richard, x="stimulus", colour_by="time", features="ENSMUSG00000092341") + 
        scale_x_discrete(guide = guide_axis(angle = 70)) +
        ggtitle("After library size normalization"),
    plotExpression(sce.spikenorm.richard, x="stimulus", colour_by="time", features="ENSMUSG00000092341") + 
        scale_x_discrete(guide = guide_axis(angle = 70)) +
...
markers1 <- scoreMarkers(logcounts(sce.libnorm.richard)["ENSMUSG00000092341",,drop=FALSE], sce.libnorm.richard$stimulus, all.pairwise=TRUE)
markers2 <- scoreMarkers(logcounts(sce.spikenorm.richard)["ENSMUSG00000092341",,drop=FALSE], sce.spikenorm.richard$stimulus, all.pairwise=TRUE)
