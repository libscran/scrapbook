...
These assumptions may not be valid in other contexts for which standard LOWESS may be more appropriate.
...
We still use the library size factors for the endogenous genes as we are not currently interested in changes in total RNA content.
...
```{r trend-plot-spike-in, fig.cap="Variance of endogenous genes (black) and spike-in transcripts (blue crosses) in the Zeisel brain dataset, as a function of the mean. Trends are fitted to the spike-in transcripts (blue full) and endogenous genes (red dashed)."}
