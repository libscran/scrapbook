...
When `block=` is set, `chooseRnaHVgs.se()` will report the average statistics across blocks for feature selection.
This ensures that each blocks contributes some information about the variability of each gene. 
We then use the average residuals to select our top HVGs as described previously.
An alternative approach would be to take the intersection or union of the HVGs from each block, but this does not scale well,
...
Back in the old days, everyone was obsessed with modelling the gene-wise variability in scRNA-seq data.
...
Also note that we only use the spike-in factors for the spike-in transcripts -
we still use the library size factors for the endogenous genes as we are not interested in changes in total RNA content.
We ensure that normalized abundances are still comparable between genes and spike-ins by centering both sets of size factors to preserve the scale of the original counts. 
