...
```{r trend-plot-pbmc, fig.cap="Variance of the log-normalized expression values across all genes in the PBMC data set, as a function of the mean. Each point represents a gene while the blue line represents the trend fitted to all genes. HVGs are represented by black points while the other genes are grey."}
library(scrapper)
...
ordered.mean.10x <- order(var.10x$statistics$means)
lines(var.10x$statistics$means[ordered.mean.10x],
    var.10x$statistics$fitted[ordered.mean.10x],
...
We'll discuss this more in Chapter \@ref(dimensionality-reduction), but it is as simple as using only the subset of HVGs in the analysis:
...
For example, heterogeneity in T cell activation responses is an interesting phenomena [@richard2018tcell]
but may be irrelevant noise in studies that only care about distinguishing the major immunophenotypes.
Our recommendation is to simply pick a reasonable $H$ (somewhere between 1000 and 5000) and proceed with the rest of the analysis.
We can always test other choices later rather than spending much time worrying about obtaining the "optimal" value.

Alternatively, we could define the set of HVGs as all genes with variances above the trend.
This avoids any judgement calls regarding what level of variation is interesting enough to retain.
By retaining all potential biological signal, we give secondary population structure the chance to manifest.
This is most useful for rare and/or weakly-separated subpopulations where the relevant marker genes are not variable enough to sneak into the top $H$ genes.
...
Setting `block=` in `modelGeneVariances()` will compute statistics and fit a separate trend within each batch.
...
for (block in names(var.tropho$per.block)) {
    ...
    ordered <- order(current$means)
    lines(current$means[ordered],
        current$fitted[ordered],
        col="dodgerblue", lwd=2)
}
```

`modelGeneVariances()` will also average statistics across batches for feature selection.
This ensures that each batch contributes some information about the variability of each gene. 
Here, we select the top 2000 HVGs based on the average residuals across the two batches:
...
```{r trend-plot-seger, fig.cap="Variance of the log-normalized expression values across all genes in one donor of the Segerstople pancreas data set, as a function of the mean. Each point represents a gene while the blue and red lines represent trends fitted with different parameters."}
...
ordered <- order(var.default.seger$statistics$means)
lines(var.default.seger$statistics$means[ordered],
    var.default.seger$statistics$fitted[ordered],
    col="dodgerblue", lwd=2)
lines(var.minw.seger$statistics$means[ordered],
    var.minw.seger$statistics$fitted[ordered],
    col="red", lwd=2)

legend("topright", lwd=2, col=c("dodgerblue", "red"), legend=c("default", "min-width"))
```
...
Fine, but such "biases" are already present at every stage, starting with experimental design and ending with the interpretation of the data.
...
# endogenous genes and the spike-ins.
...
This assumes that the spike-ins and endogenous genes are both affected by the same technical factors.
...
plot(var.gene.zeisel$statistics$means, var.gene.zeisel$statistics$variances,
    xlab="Mean of log-expression", ylab="Variance of log-expression",
    pch=16, cex=0.5)
ordered.mean.gene.zeisel <- order(var.gene.zeisel$statistics$means)
lines(var.gene.zeisel$statistics$means[ordered.mean.gene.zeisel],
    var.gene.zeisel$statistics$fitted[ordered.mean.gene.zeisel],
    col="dodgerblue", lwd=2)

points(var.spike.zeisel$statistics$means, var.spike.zeisel$statistics$variances, pch=4)
ordered.mean.spike.zeisel <- order(var.spike.zeisel$statistics$means)
lines(var.spike.zeisel$statistics$means[ordered.mean.spike.zeisel],
    var.spike.zeisel$statistics$fitted[ordered.mean.spike.zeisel],
    col="red", lwd=2, lty=2)
...
```{r}
tech.var.zeisel <- approx(var.spike.zeisel$statistics$means,
    var.spike.zeisel$statistics$variances,
    xout=var.gene.zeisel$statistics$means, rule=2)$y
