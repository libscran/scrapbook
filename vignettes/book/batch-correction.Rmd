...
cluster.batch.mnn.pbmc <- table(sce.mnn.graph.pbmc$clusters, sce.mnn.graph.pbmc$batch)
...
cluster.batch.mnn.pbmc <- t(t(cluster.batch.mnn.pbmc) / colSums(cluster.batch.mnn.pbmc))
cluster.batch.mnn.pbmc / rowSums(cluster.batch.mnn.pbmc)
```
...
cluster.batch.unc.pbmc <- table(sce.unc.graph.pbmc$clusters, sce.unc.graph.pbmc$batch)

cluster.batch.unc.pbmc <- t(t(cluster.batch.unc.pbmc) / colSums(cluster.batch.unc.pbmc))
cluster.batch.unc.pbmc / rowSums(cluster.batch.unc.pbmc)
```
...
This, in turn, allows us test for differences in expression or abundance of the same cell type/state across conditions (Section \@ref(multi-condition-analysis)).
...
Any differences between conditions will still be preserved in the results of Section \@ref(multi-condition-analysis).
...
cluster.batch.mnn.pancreas <- table(sce.nn.mnn.pancreas$clusters, sce.nn.mnn.pancreas$batch)
cluster.batch.mnn.pancreas <- t(t(cluster.batch.mnn.pancreas) / colSums(cluster.batch.mnn.pancreas))
cluster.batch.mnn.pancreas / rowSums(cluster.batch.mnn.pancreas)
...
gridExtra::grid.arrange(
    plotExpression(sce.nn.mnn.pancreas, x="clusters", features=current.insigf2, other_fields="batch") + 
        facet_grid(~batch) +
        ggtitle("original expression"),
    plotXY(sce.nn.mnn.pancreas$clusters, lowrank.unc.insigf2, other_fields=list(batch=sce.nn.mnn.pancreas$batch)) + 
        facet_grid(~batch) +
        ggtitle("reconstruction without correction"),
    plotXY(sce.nn.mnn.pancreas$clusters, lowrank.mnn.insigf2, other_fields=list(batch=sce.nn.mnn.pancreas$batch)) +
        facet_grid(~batch) +
...
Consider a multi-condition analysis that already requires multiple replicate samples for each condition.
