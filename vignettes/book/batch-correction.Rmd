...
This benefit is probably not that reliable as it assumes that the correction algorithm manages to preserve the rare subpopulations.
...
```{r batch-activity-example, echo=FALSE, fig.wide=TRUE, fig.height=6, fig.width=10, fig.cap="Diagram of MNN correction when the batch effect is confounded with biological variation."} 
set.seed(999)
N <- 200
x1 <- runif(N) * 4 * pi
...
Admittedly, we're misusing the word "batch" here, but we're already halfway into this chapter and we don't want to rewrite the text so just bear with it until the end.
