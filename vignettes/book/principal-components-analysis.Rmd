...
reducedDim(sce.qc.416b, "PCA.unblocked") <- t(pcs.raw.416b$components)
pcs.blocked.416b <- runPca(logcounts(sce.qc.416b)[hvg.416b,], number=20, block=sce.qc.416b$batch)
reducedDim(sce.qc.416b, "PCA.blocked") <- t(pcs.blocked.416b$components)

library(scater)
gridExtra::grid.arrange(
    plotReducedDim(sce.qc.416b, dimred="PCA.unblocked", colour_by="batch") +
        ggtitle("Without blocking"),
    plotReducedDim(sce.qc.416b, dimred="PCA.blocked", colour_by="batch") +
        ggtitle("Blocked"),
    ncol=1
)
```
...
reducedDim(sce.qc.416b, "PCA.residuals") <- t(pcs.residuals.416b$components)

library(scater)
plotReducedDim(sce.qc.416b, dimred="PCA.residuals", colour_by="batch")
```
